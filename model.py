...
class ComplexLiteral_attention(torch.nn.Module):

    def __init__(self, num_entities, num_relations, numerical_literals):
        super(ComplexLiteral_attention, self).__init__()
    ...
    def forward(self, e1, rel):
        ...
        e1_emb_real = self.emb_num_lit_real(torch.cat([e1_emb_real, e1_num_weighted_literal], 1))
        e1_emb_img = self.emb_num_lit_img(torch.cat([e1_emb_img, e1_num_weighted_literal], 1))

        e2_num_weighted_literal = self.numerical_literals * self.entity_specific_literal.weight
        e2_multi_emb_real = self.emb_num_lit_real(torch.cat([self.emb_e_real.weight, e2_num_weighted_literal], 1))
        e2_multi_emb_img = self.emb_num_lit_img(torch.cat([self.emb_e_img.weight, e2_num_weighted_literal], 1))
